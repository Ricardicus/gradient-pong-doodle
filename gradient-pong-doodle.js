...
var targetAccelerationTop = 0.00001;
var accelerationPeakReached = false;
var targetMaxSpeed = 0.3; // after this, linearly increase speed slowly
...
function updateLearningRateBar(arm) {
  document.getElementById(
    "arm-" + (arm + 1) + "-label-learning-rate",
  ).innerHTML =
    "Learning rate: " + Number.parseFloat(learningRate[arm]).toPrecision(4);
...
function newOptStrat(arm) {
  if (document.getElementById("arm-" + (arm + 1) + "-adam-select").checked) {
    ...
    document.getElementById("arm-" + (arm + 1) + "-adagrad-select").checked
  ) {
    learningRate[arm] = 0.3943;
    updateLearningRateBar(arm);
...
function newADAMBeta1(arm) {
  ...
  adam_beta_1[arm] = Math.pow(
    10,
    minv +
      scale *
        document.getElementById("arm-" + (arm + 1) + "-adam-beta-1").value,
  );
  document.getElementById("arm-" + (arm + 1) + "-label-adam-beta-1").innerHTML =
    "ADAM β1: " + Number.parseFloat(adam_beta_1[arm]).toPrecision(4);
}
...
function newADAMBeta2(arm) {
  ...
  adam_beta_2[arm] = Math.pow(
    10,
    minv +
      scale *
        document.getElementById("arm-" + (arm + 1) + "-adam-beta-2").value,
  );
  document.getElementById("arm-" + (arm + 1) + "-label-adam-beta-2").innerHTML =
    "ADAM β2: " + Number.parseFloat(adam_beta_2[arm]).toPrecision(4);
}
...
function newOmegas(arm) {
  for (var i = 0; i < omegas[0].length; i++) {
    omegas[arm][i] = document.getElementById(
      "arm-" + (arm + 1) + "-omega-" + (i + 1),
    ).value;
  }
}
...
function de_dpnx_domega_q(omegas, radiuses, R, q, n) {
  if (q > n) return 0.0;
  return (
    de_dpnx_domega_q(omegas, radiuses, R, q, n - 1) -
    radiuses[n - 1] *
      0.01 *
      R *
      Math.sin(omega_sum_order(n, omegas)) *
      Math.PI *
      2 *
      0.01
  );
...
function de_dpny_domega_q(omegas, radiuses, R, q, n) {
  if (q > n) return 0.0;
  return (
    de_dpny_domega_q(omegas, radiuses, R, q, n - 1) +
    radiuses[n - 1] *
      0.01 *
      R *
      Math.cos(omega_sum_order(n, omegas)) *
      Math.PI *
      2 *
      0.01
  );
...
function de_domega_q(point, target, radiuses, omegas, R, q) {
  var n = omegas.length;
  var dxp = point[0] - target[0];
  if (Math.abs(dxp) > gradientClip) {
    // clipping the gradient by limiting the x distance
    dxp = gradientClip * Math.sign(dxp);
  }
  return (
    dxp * de_dpnx_domega_q(omegas, radiuses, R, q, n) +
...
function populateValues() {
  for (var arm = 0; arm < 2; arm++) {
    ...
    for (var i = 0; i < omegas[0].length; i++) {
      newHTML +=
        '<input type="range" oninput="newOmegas(' +
        arm +
        ');"\
         min="0" max="100" id="arm-' +
        (arm + 1) +
        "-omega-" +
        (i + 1) +
        '">\
  <label id="arm-' +
    ...
    document.getElementById("arm-" + (arm + 1) + "-omega-bars").innerHTML =
      newHTML;
    document.getElementById(
      "arm-" + (arm + 1) + "-label-learning-rate",
    ).innerHTML =
      "Learning rate: " + Number.parseFloat(learningRate[arm]).toPrecision(4);
    document.getElementById(
      "arm-" + (arm + 1) + "-label-adam-beta-1",
    ).innerHTML =
      "ADAM β1: " + Number.parseFloat(adam_beta_1[arm]).toPrecision(4);
    document.getElementById(
      "arm-" + (arm + 1) + "-label-adam-beta-2",
    ).innerHTML =
      "ADAM β2: " + Number.parseFloat(adam_beta_2[arm]).toPrecision(4);
...
function getPoints(arm) {
  ...
  for (var n = 1; n <= omegas[arm].length; n++) {
    points.push([
      parseInt(
        startPos[arm][0] * 0.01 * XTot +
          p_n_x(omegas[arm], radiuses[arm], R, n),
      ),
      parseInt(
        startPos[arm][1] * 0.01 * YTot +
          p_n_y(omegas[arm], radiuses[arm], R, n),
      ),
    ]);
  }
  return points;
...
function updateOmegas(arm, points) {
  ...
  for (var i = 0; i < omegas[arm].length; i++) {
    document.getElementById("arm-" + (arm + 1) + "-omega-" + (i + 1)).value =
      omegas[arm][i];
  }
  if (arm == 0 && dx > 0) {
  ...
  if (error <= optimizationStop[arm]) {
    if (accelerationPeakReached == false) {
      dx = dx * targetAcceleration; // going faster and faster
      if (Math.abs(dx) > targetMaxSpeed) {
        dx = targetMaxSpeed * Math.sign(dx);
      }
    } else {
      dx = (Math.abs(dx) + targetAccelerationTop) * Math.sign(dx);
    }
    if (Math.abs(dx) >= targetMaxSpeed) {
      accelerationPeakReached = true;
    } else {
      accelerationPeakReached = false;
    }
    dx = -1 * dx;
  ...
  document.getElementById("arm-" + (arm + 1) + "-loss").innerHTML =
    Number.parseFloat(error).toPrecision(4);
  var de_domegas = [];
  for (var i = 0; i < omegas[arm].length; i++) {
    var de_domega_i = de_domega_q(
      point,
      target,
      radiuses[arm],
      omegas[arm],
  ...
  if (optimizationStrategy[arm] == BASIC) {
    for (var i = 0; i < omegas[arm].length; i++) {
      omegas[arm][i] =
        omegas[arm][i] -
        learningRate[arm] * de_domegas[i] -
        lambda * Math.abs(omegas[arm][i]);
    }
  } else if (optimizationStrategy[arm] == ADAM) {
    for (var i = 0; i < omegas[arm].length; i++) {
      adam_v[arm][i] =
        adam_beta_2[arm] * adam_v[arm][i] +
        (1.0 - adam_beta_2[arm]) * Math.pow(de_domegas[i], 2);
      adam_momentum_omegas[arm][i] =
        adam_beta_1[arm] * adam_momentum_omegas[arm][i] +
        (1.0 - adam_beta_1[arm]) * de_domegas[i];
      vhat = adam_v[arm][i] / (1.0 - adam_beta_2[arm]);
      mhat = adam_momentum_omegas[arm][i] / (1.0 - adam_beta_1[arm]);
      omegas[arm][i] =
        omegas[arm][i] -
        (learningRate[arm] * mhat) / (Math.sqrt(vhat) + adam_epsilon);
    }
    ...
    for (var i = 0; i < omegas[arm].length; i++) {
      ...
      omegas[arm][i] =
        omegas[arm][i] -
        learningRate[arm] *
          ((1.0 / Math.sqrt(adagrad_G[arm][i] + epsilon)) * de_domegas[i]);
    }
  } else {
    console.log("no opt..");
  }
...
function debugPrint(arm) {
  for (var i = 0; i < omegas[arm].length; i++) {
    document.getElementById(
      "arm-" + (arm + 1) + "-label-bar-" + (i + 1),
    ).innerHTML =
      "Ω" +
      (i + 1) +
      ": " +
      Number.parseFloat(omegas[arm][i] * 0.01 * 360).toFixed(2) +
      " °";
  }
}
...
function updateScoreBoard() {
  document.getElementById("scoreboard").innerHTML =
    "Score: " + scoreboard[0] + " - " + scoreboard[1];
}
...
function drawArm(ctx, arm, points) {
  ctx.beginPath();
  ctx.moveTo(
    parseInt(startPos[arm][0] * XTot * 0.01),
    parseInt(startPos[arm][1] * YTot * 0.01),
  );
...
function drawTarget(ctx) {
  ...
  ctx.beginPath();
  ctx.arc(
    targetPos[0] * 0.01 * XTot,
    targetPos[1] * 0.01 * YTot,
    4,
    0,
    2 * Math.PI,
    false,
...
function pongDoodleToggleTheme() {
